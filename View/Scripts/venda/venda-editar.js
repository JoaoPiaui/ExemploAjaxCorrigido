$(function () {
    ...
    $tabela = $("#venda-produtos-index").DataTable({
        ajax: "/produto/obtertodospeloidvenda?idVenda=" + $idVenda, serverSide: true, columns: [
            { data: "Nome" },
            { data: "Quantidade" },
            { data: "Valor" },
            {
    ...
    function alterar($nome, $quantidade, $valor) {
        $.ajax({
            ...
            method: "post",
            data: {
                nome: $nome, quantidade: $quantidade, valor: $valor, id: $idAlterar, idVenda: $idVenda
            },
            success: function (data) {
                $("#venda-produto-modal").modal("hide");
                limparCampos();
